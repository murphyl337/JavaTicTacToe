...
public class GameRules {
	...
	public boolean isWinnerHorizontally(String marker) {
		...
		for (int row = 0; row < 3; row++) {
			if (board.getSpace(row,0).equals(marker) && board.getSpace(row, 1).equals(marker)
	...
	public boolean isWinnerVertically(String marker) {
		...
		for (int col = 0; col < 3; col++) {
			if (board.getSpace(0, col).equals(marker) && board.getSpace(1, col).equals(marker)
	...
	public boolean isWinnerDiagonally(String marker) {
		boolean winner = false;
		if (board.getSpace(0,0).equals(marker) && board.getSpace(1,1).equals(marker)
				&& board.getSpace(2,2).equals(marker))
			winner = true;

		if (board.getSpace(0,2).equals(marker) && board.getSpace(1,1).equals(marker)
